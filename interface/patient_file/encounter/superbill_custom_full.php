...
//ALB Added global periods, effective date, and cosmetic to the Codes screen
...
if (isset($mode) && $thisauthwrite) {
    ...
    $cosmetic = empty($_POST['cosmetic']) ? 0 : 1;
    $global = $_POST['global'];
    ...
    if ($mode == "delete") {
        ...
        if ($crow['count']) {
            ...
            $sql =
                ...
                "eff_date = '"     . ffescape($eff_date)      . "', " . //ALB Added these
                "cosmetic = '"     . ffescape($cosmetic)  . "', " .
                "global = '"       . ffescape($global) . "', " .
...
<form method='post' action='superbill_custom_full.php' name='theform'>
    ...
    <div class="container">
      ...
      <div class="form-group row">
        ...
        <div class="col-md"> <!--ALB Added these -->
          <?php echo xlt('Global Period') . "(" . xlt('days') ."):"; ?>
          <input type='text' size='3' name='global' value='<?php echo attr($global) ?>'>
        </div>
        <div class="col-md">        
          <input type='checkbox' name='cosmetic' value='1'<?php if (!empty($cosmetic)) echo ' checked'; ?> />
          <?php echo xlt('Cosmetic'); ?>
        </div>
