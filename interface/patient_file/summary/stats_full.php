        ...
        <div id='patient_stats'>
            <form method='post' action='stats_full.php' onsubmit='return top.restoreSession()'>
                <div class="table-responsive">
                    ...
                    //ALB Adding colored problems for different outcomes
                    $color = '';
                    if ($focustype == "medical_problem") {
            		...
            		while ($temprow = sqlFetchArray($tempres)) {
              		  if ($temprow['notes'] <> '') $color = $temprow['notes'];
                        }
                    }
                                ...
                                //ALB Change color here
                                echo "  <td class='text-left " . attr($click_class) . "' style='text-align:left " . ($color <> '' ? ';color:$color;font-weight:bold' : '') ."' id='" . attr($rowid) . "'>" . text($disptitle) . "</td>\n";
