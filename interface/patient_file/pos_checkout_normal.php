                ...
                <div class="col-6 offset-lg-3">
                    <table class="table">
                        ...
                        while ($inrow = sqlFetchArray($inres)) {
                            ...
                            receiptDetailLine(
                                $svcdate,
                                //ALB Added code here to the description
                                $inrow['code'] . ": " . $inrow['code_text'],
                                $inrow['fee'],
                                $inrow['units']
                            );
                        }
                        ...
                        <tr>
                            ...
                            <td class='text-right'>&nbsp;</td>
                            <td class='text-right'><?php echo text(oeFormatMoney($charges, false)) ?></td> <!--ALB Changed to false here -->
                        </tr>
                        <tr>
                            <td colspan='5'>&nbsp;</td>
                        </tr>
                        <?php
                        // Get co-pays.
                        //ALB Changed date to display the date when the copay was applied
                        ...
                        while ($inrow = sqlFetchArray($inres)) {
                            $charges += sprintf('%01.2f', $inrow['fee']);
                            //ALB This is new
                            $copay_date = substr($inrow['date'],0,10);
                            //receiptPaymentLine($svcdate, 0 - $inrow['fee'], $inrow['code_text']);
                            receiptPaymentLine($copay_date, 0 - $inrow['fee'], $inrow['code_text']);

                        }
                        // Get other payments.
                        $inres = sqlStatement("SELECT " .
                        "a.code_type, a.code, a.modifier, a.memo, a.payer_type, a.adj_amount, a.pay_amount, a.post_time, " .
                        "s.payer_id, s.reference, s.check_date, s.deposit_date, s.payment_method " . //ALB Added a few columns
                        "FROM ar_activity AS a " .
                        "LEFT JOIN ar_session AS s ON s.session_id = a.session_id WHERE " .
                        "a.pid = ? AND a.encounter = ? AND a.deleted IS NULL AND " .
                        "a.pay_amount != 0 " .
                        ...
                        while ($inrow = sqlFetchArray($inres)) {
                            ...
                            //ALB Changed date to display the check date (or post date is check date is empty) when the payment was issued from $svcdate
                            receiptPaymentLine(
                                ($inrow['check_date'] ? $inrow['check_date'] : substr($inrow['post_time'],0,10)),
                                $inrow['pay_amount'],
                                $payer . ' ' . ($payer == 'Pt' ? ($inrow['payment_method'] ? $inrow['payment_method'] : $inrow['memo']) : $inrow['reference'])
                            );
...
?>
    ...
    function write_form_line(
        ...
        echo "</td>\n";
        //ALB Added codes to the descriptions
        echo "  <td>" . text($code) . ": " . text($description) . "</td>";
    ...
    if (!empty($_POST['form_save'])) {
      ...
      // Post payment.
        //ALB Only post payment if > 0
        if ($_POST['form_amount'] && ($_POST['form_amount'] !='0.00')) {
    ...
    $query = "SELECT id, date, code_type, code, modifier, code_text, " .
      ...
      //"billed = 0 AND  //ALB Not just unbilled 
      "code_type != 'TAX' " .
    ...
    <head>
        ...
        <script>
            ...
            function computeTotals() {
                ...
                f.form_amount.value = '0.00';  //ALB Want to default to 0total.toFixed(<?php echo js_escape($currdecimals); ?>);
                return true;
            }
        ...
        <div id="container_div" class="<?php echo $oemr_ui->oeContainer();?> mt-3">
            ...
            <div class="row">
                <div class="col-sm-12">
                            ...
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    while ($brow = sqlFetchArray($bres)) {
                                        //ALB If already on an encounter, complete checkout for that encounter, otherwise complete the most recent one
  					if (!$encounter) {
  						// Skip all but the most recent encounter.
    						if ($inv_encounter && $brow['encounter'] != $inv_encounter) continue;
 					} else {
  						//ALB Skip all but the current encounter
  						if ($brow['encounter'] != $encounter) continue;
					...
					//ALB If Code_Type is CPT, use the date for the encounter, not the date when the charge was entered (sometimes may be entered later, but still for the same encounter).
  					$line_date = $thisdate;
  					$current_enc = ($inv_encounter ? $inv_encounter : $brow['encounter']);

  					if (substr($code_type,0,3) == 'CPT') {
    						$equery = "SELECT date FROM form_encounter WHERE " .
    						"pid = '$patient_id' AND encounter = '$current_enc' " .
    						"ORDER BY id DESC LIMIT 1";
    						$etemp = sqlQuery($equery);
    						$enc_date = substr($etemp['date'],0,10);
    						$line_date = $enc_date;
  					}

                                        //ALB Changed from $thisdate to $line_date below
                                        write_form_line(
                                            $code_type,
                                            $brow['code'],
                                            $brow['id'],
                        ...
                        <fieldset>
                            ...
                            <div class="col-12 oe-custom-line">
                                ...
                                <div class="col-3 offset-lg-3"> <!--ALB Changed formatting here -->
                            ...
                            <div class="col-12 oe-custom-line">
                                ...
                                <div class="col-3 offset-lg-3"> <!--ALB Changed here -->
