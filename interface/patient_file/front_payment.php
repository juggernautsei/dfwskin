...
function echoLine($iname, $date, $charges, $ptpaid, $inspaid, $duept, $encounter = 0, $copay = 0, $patcopay = 0)
{
    global $var_index;
    $var_index++;
    //ALB Took out bucks here, as it gets formatted later on
  ...
  //ALB If no charges entered for today, display that
  if ($date == "Charges have not been entered yet") {
    echo " <tr id='tr_" . attr($var_index) . "' >\n";
    echo "  <td class='text-center'>" . "Today" . "</td>\n";
    echo "  <td class='text-center' id='" . attr($today) . "' >" . text($encounter) . "</td>\n";
    echo "  <td class='text-center' id='td_charges_$var_index' >" . text($date) . "</td>\n";
  } else {
    ...
    //ALB Don't display the required copay, because it may not be required
...
if (!empty($_POST['form_save'])) {
    ...
    $NameNew = 'COPAY'; // ALB Don't need to store the entire name in the description of payment   
        ...
        foreach ($_POST['form_upay'] as $enc => $payment) {
            ...
            if ($amount = $payment) {
                ...
                if ($_REQUEST['radio_type_of_payment'] == 'invoice_balance' || $_REQUEST['radio_type_of_payment'] == 'cash') {                //Payment by patient after insurance paid, cash patients similar to do not bill insurance in feesheet.
                    //ALB Don't need to do this if someone paid cash
                    ...
                    //--------------------------------------------------------------------------------------------------------------------
                        ...
                        //ALB The statement count and date for previous encounters get reset, but only for those statements that have previously been sent out to patients (so that those encounters that are still due insurance don't get a statement date).
                        sqlStatement(
                            "UPDATE form_encounter SET last_stmt_date = '". date('Y-m-d') . "', stmt_count = '0' WHERE last_stmt_date != '' AND pid=? ",
                            array($form_pid)
                        );
...
function CheckVisible(MakeBlank) { //Displays and hides the check number text box.
  //ALB Making this box available for cc payments
  if (document.getElementById('form_method').options[document.getElementById(
            'form_method').selectedIndex].value == 'cash') {    
            ...
            <div class="col-sm-12">
                <form class="form form-vertical" method='post' action='front_payment.php<?php echo (!empty($payid)) ? "?payid=" . attr_url($payid) : ""; ?>' onsubmit='return validate();'>
                    ...
                    <fieldset>
                        ...
                        <!-- ALB Made this next session hidden -->
                        <div class="col-12 oe-custom-line" hidden>
                        ...
                        <div class="col-12 oe-custom-line">
                            ...
                            <div id="tr_radio2" style="padding-left:15px"><!-- For radio self -->
                                <!--ALB Don't need this  <label class="radio-inline">
                    ...
                    <fieldset>
                        ...
                        <div class="table-responsive">
                            <table class="table" id="table_display">
                                ...
                                //
                                if (!$gottoday) {
                                    //ALB Don't need to do this here echoLine("form_upay[0]", date("Y-m-d"), 0, 0, 0, 0 /*$duept*/);//No encounter yet defined.
                                }

                                //$gottoday = false; ALB Changed this
                                foreach ($encs as $key => $value) {
                                    $enc = $value['encounter'];
                                    $dispdate = $value['date'];
                                    if (strcmp($dispdate, $today) == 0 && !$gottoday) {
                                    ...
                                    $drow = sqlQuery(
                                        ...
                                        "SUM(adj_amount) AS adjustments  FROM ar_activity WHERE " .
                                        "deleted IS NULL AND pid = ? and encounter = ? and " .
                                        "payer_type = 0", //ALB changed query to pull all patient payments and account_code!='PCP' ",
                                        array($pid, $enc)
                                    );
                                    ...
                                    //ALB Skip if no charges yet
                                        if (!$gottoday) {
                                           $dispdate = "Charges have not been entered yet";
                                        } else {
      					   //ALB If visit is today, get the copay, but only if it is greater than the total charges due for the visit and has not yet been paid.
                                           $cur_balance = $value['charges'] - $dpayment_pat - $dpayment - $dadjustment;
				           if ((!$dpayment_pat || $dpayment_pat == 0) && $inscopay > 0) {
           					$duept = (($inscopay <= $cur_balance) ? $inscopay : $cur_balance);
        				   }
					   ...
					   $cosm_query = sqlQuery("SELECT c.cosmetic FROM codes as c, " .
    				           ...
    				           if ($cosm_query['cosmetic'] == 1) $duept = $cur_balance;                                             
                                        }
                                    }
                                ...
                                <tr class="table-active">
                                    ...
                                    <!-- ALB Took 2 columns out here <td class="font-weight-bold" id='td_total_7'></td>
                                    <td class="font-weight-bold" id='td_total_8'></td> -->
                                    <td class="font-weight-bold text-right"><?php echo xlt('Total');?></td>
                                    <td class="font-weight-bold text-right"> <!--ALB Change width here -->
                                        <input type='text' class='form-control text-success' name='form_paytotal' style='color:#00aa00;width:80px' value='' readonly />
                                    </td>
                                </tr>
                            </table>
                        </div>
