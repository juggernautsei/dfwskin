...
if ($transid) {
    ...
    //ALB Adding risk level
    $risk_level = $trow['refer_risk_level'];
    //ALB Adding comments (additional notes) to the consult
    $refer_comments1 = "";
    $refer_comments2 = wordwrap($trow['refer_comments'],100);
...
if ($patient_id) {
    ...
    //ALB Pull patient phone number and risk level
    $risk_level = (Is_Null($risk_level) ? "Normal" : $risk_level);
    $patient_phone = ($patdata['phone_home'] ? $patdata	['phone_home'] : $patdata['phone_cell']);
...
//ALB Adding these
$s = str_replace("{pt_phone}", text($patient_phone), $s);
$s = str_replace("{ref_refer_risk_level}", text($risk_level), $s);
$s = str_replace("{ref_provider_NPI}", text($provider_NPI), $s);
$s = str_replace("{ref_comments1}", text($refer_comments1), $s);
$s = str_replace("{ref_comments2}", text($refer_comments2), $s);
$s = str_replace("{ref_comments3}", text($refer_comments3), $s);
$s = str_replace("{ref_comments4}", text($refer_comments4), $s);
