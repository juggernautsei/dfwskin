...
//ALB Added facility address to the envelopes
$facdata = sqlQuery("SELECT " .
  "f.name, " .
  "f.street, f.city, f.state, f.postal_code " .
  "FROM facility AS f " .
  "WHERE f.billing_location = 1 LIMIT 1");
$text4 = sprintf("%s \n", $facdata['name']);
$text5 = sprintf("%s \n", $facdata['street']);
$text6 = sprintf("%s, %s %s", $facdata['city'], $facdata['state'], $facdata['postal_code']);

//ALB This adds the facility address to the envelope
$pdf = new eFPDF('P', 'mm', array($x_width, $y_height)); // set the orentation, unit of measure and size of the page
$pdf->AddPage();
$pdf->SetFont('Arial', '', $font_size-4);
$pdf->TextWithRotation($x-50, $y + 80, $text4, $angle);
$pdf->TextWithRotation($x-45, $y + 80, $text5, $angle);
$pdf->TextWithRotation($x-40, $y + 80, $text6, $angle);
