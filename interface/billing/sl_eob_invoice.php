...
// This should not happen but if it does, apply old logic.
            ...
            //ALB Adding the ability to manually add claim number here
            $claim_number = '';
            if ($_POST['form_insurance'] == 'Ins1') {
                $claim_number = $_POST['form_claim_number_1'];
            }
            if ($_POST['form_insurance'] == 'Ins2') {
                $claim_number = $_POST['form_claim_number_2'];
            }
            if ($_POST['form_insurance'] == 'Ins3') {
                $claim_number = $_POST['form_claim_number_3'];
            }
...
// Ins1, Ins2 or Ins3.
                ...
                SLEOB::arPostAdjustment($patient_id, $encounter_id, $session_id, $thisadj, $code, $payer_type, $reason, $debug, '', $thiscodetype, $claim_number);
            }
        }
        ...
        <form class="form" action='sl_eob_invoice.php?id=<?php echo attr_url($trans_id); ?>' method='post' onsubmit='return validate(this)'>
            ...
            <fieldset>
                ...
                <div class="form-row">
                        ...
                        <div class="pl-3">
                            <?php
                                ...
                                foreach (array(1 => 'Ins1', 2 => 'Ins2', 3 => 'Ins3', 4 => 'Pt') as $key => $value) {
                                    ...
                                    $post_to = ($last_level_closed > 3 ? $last_level_closed : $last_level_closed + 1 + $skipped); //ALB Unless already on patient, set Now Posting to to the next ins
                                    $checked = ($post_to == $key) ? " checked" : "";
                                    echo "<label class='radio-inline'>&nbsp; ";
                                    echo "<input type='radio' name='form_insurance' onclick='setins('" . attr($value) . "')' value='" . attr($value) . "'$checked />" . text($value);
                                    echo "</label>";
                                }
                            ?>

                            <!--ALB Deleted all below as described above
                            ?>
                    ...
                    <!--ALB Adding claim numbers here for every insurance that patient has --> 

                         <?php
                          foreach (array(1 => 'Ins1', 2 => 'Ins2', 3 => 'Ins3') as $key => $value) {
                              if ($key && !SLEOB::arGetPayerID($patient_id, $svcdate, $key)) {
                                   continue;
                              }
                              $claim_number = '';
                              $tmp = sqlQuery("SELECT claim_number " .
                                  "FROM ar_activity WHERE code != 'CO-PAY' AND " .
                                  "pid = ? AND encounter = ? AND payer_type = ? " . 
                                  "ORDER BY claim_number DESC LIMIT 1", array($patient_id, $encounter_id, $key));
                              $claim_number = $tmp['claim_number'];
                         ?>
                    <div class="col-xs-2">
                        <label class="control-label" for="form_claim_number"><?php echo xlt('Claim number for Ins') . $key ; ?>:</label>
                        <input type='text' name='<?php echo "form_claim_number_" . $key; ?>' id='<?php echo "form_claim_number_" . $key; ?>' class="form-control" value='<?php echo $claim_number; ?>' />                        
                    </div>
                        <?php
                            if ($claim_number) {
                               //ALB if there is already a claim number for a specific insurance, disable the textbox
                               $tempid = "form_claim_number_" . $key;
                               ?>
                               <script>
                        ...
                        ?>
                    <!--ALB This is where the claim number text boxes end -->
