...
<head>
    ...
    <script>
        ...
        //ALB Need to be able to re-open, even if prev billed (that is, if there is a secondary, but need to correct for primary, need to be able to reopen
        ...
        // var can_bill = (count0 == 0 && count1 == 0 && count2 > 0);
            ...
            f.bn_reopen.disabled = 0; //ALB I want to be able to reopen any claim if needed!can_bill;
            <?php } ?>
                ...
                <form class="form" name='the_form' method='post' action='billing_report.php' onsubmit='return top.restoreSession()'>
                    ...
                    if ($daysheet) {
                        ...
                        if (!isset($_REQUEST['mode'])) {// default case
                            ...
                            //ALB Don't need unbilled only here - three lines commented out below
    ...
    <div class="container-fluid mt-1">
        <form class="form-inline" name='update_form' method='post' action='billing_process.php'>
            ...
            <div class="table-responsive">
                    ...
                    <?php } ?>
                        ...
                        // billing items, then it is not of interest.
                            ...
                            // This dumps all HTML for the previous encounter.
                                ...
                                // Is there a MBO
                                // ALB Took out forms.authorized = 1 below
				...
				//ALB Need to find the current primary insurance, not just any
                                //$res = sqlQuery(
				...
				$namecolor = ($res['provider']) ? "black" : "#ff7777";
                                ...
                                //ALB $bgcolor = ((($encount ?? null) & 1) ? "var(--light)" : "var(--gray300)");
                    ...
                    <script>
                        ...
                        EncounterNoteArray[<?php echo attr($iter['enc_pid']); ?>] = new Array;
                                ...
                                //ALB Making more compact here
                                //$lhtml .= "</div>";

                                $divnos = $divnos + 1;

                                //ALB Don't need it here, I prefer a more compact look
				    ...
				    //ALB To account for patients with no insurance, need to create a left join query
         				...
         				"id.pid = ? AND " .
          				...
          				"ORDER BY id.type ASC, id.date DESC";
                                    ...
                                    while ($crow = sqlFetchArray($cres)) {
                                      ...
                                      //ALB Only add to ltml and line number if notes are to be displayed, otherwise blank lines are added
                                      if ($GLOBALS['notes_to_display_in_Billing'] == 2 || $GLOBALS['notes_to_display_in_Billing'] == 3) {
                            ...
                            //ALB Show all here, add cosmetics, 0 cost visit and uninsured with red color for visibility when billing
                            //if ($iter['id'] && $iter['fee'] > 0) {
                            //    $rhtml .= text(oeFormatMoney($iter['fee']));
                            if ($iter['id']) { 
     				if (substr($iter['code_type'],0,3) == 'CPT') {
        				$tres = sqlQuery("SELECT cosmetic FROM codes WHERE " .
        					"code = '" . $iter['code'] . "' LIMIT 1");
      					if ($tres['cosmetic'] == 1) {  //ALB Cosmetic - turn red
         					$rhtml .= "<font color='red'>" . oeFormatMoney($iter['fee']) . "</font>";
				        } elseif ($iter['fee'] == 0) { //ALB No fee for procedure
         					$rhtml .= "<font color='red'>" . oeFormatMoney($iter['fee']) . "</font>";
				      	} elseif ($namecolor <> 'black') { //ALB No insurance
         					$rhtml .= "<font color='red'>" . oeFormatMoney($iter['fee']) . "</font>";
				        } elseif ($iter['fee'] > 0) { //ALB All others show in bk
                            ...
                            if ($iter['id']) {
                                ...
                                $curr_provider = getProviderName(empty($iter['provider_id']) ? text($iter['enc_provider_id']) : text($iter['provider_id']));
                                if ($prev_provider != $curr_provider) {
                                   $rhtml .= $curr_provider; //getProviderName(empty($iter['provider_id']) ? text($iter['enc_provider_id']) : text($iter['provider_id']));
