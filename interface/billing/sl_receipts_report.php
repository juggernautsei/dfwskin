...
//ALB A few more vars
$form_details  = empty($_POST['form_details']) ? 0 : 1;
$form_ins  = empty($_POST['form_ins']) ? 0 : 1;
...
<div class="container">
    ...
    <div class="row">
        <div class="col-12">
            ...
            //ALB Added 3 vars below for payer type (e.g. insurance 1, 2, or 3)
            $arows[$key]['payer_no'] = empty($row['payer_type']) ? 0 : $row['payer_type'];
            $arows[$key]['pid'] = $row['pid'];
            $arows[$key]['date'] = substr($row['date'], 0, 10);
                        ...
                        // Get insurance company name
                            $insconame = '';
                            if ($row['project_id'] > 0) {
                            //ALB if ($form_proc_codefull  && $row['project_id']) {
                                $tmp = sqlQuery("SELECT name FROM insurance_companies WHERE " .
                                "id = ?", array($row['project_id']));
                                $insconame = $tmp['name'];
                } else {
                //ALB Get insurance company from insurance data
                $insseqno = $row['payer_no'];
                $payer_type = '';
                if ($insseqno != 0) {
                   if ($insseqno == 1) $payer_type = 'primary';
                   if ($insseqno == 2) $payer_type = 'secondary';
                   if ($insseqno == 3) $payer_type = 'tertiary';

                   $tmp = sqlQuery("SELECT provider FROM insurance_data WHERE " .
                     "pid = ? AND date <= ? AND type = ? ORDER BY date DESC LIMIT 1", array($row['pid'], $row['date'], $payer_type));

                   $inscoid = ($tmp['provider'] ? $tmp['provider'] : 0);
                   $tmp = sqlQuery("SELECT name FROM insurance_companies WHERE " .
                     "id = ?", array($inscoid));
                   $insconame = ($tmp['name'] ? $tmp['name'] : 'Unknown');
                } else {
                   $insconame = 'Patient';
                }
            }
