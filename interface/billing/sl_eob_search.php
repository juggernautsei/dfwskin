    ...
    for ($rcnt = 0; $row = $rows[$rcnt] ?? null; $rcnt++) {
        ...
        foreach ($invlines as $key => $value) {
            ...
            if ($GLOBALS['use_custom_statement']) {
                $line['desc'] = ($key == 'CO-PAY') ? "Patient Payment" : "$key: " . $value['code_text']; //ALB Added code here
            } else {
        ...
        if (!$DEBUG && empty($_REQUEST['form_without'])) {
           //ALB Will perform one update for all encounters later in the code
           //sqlStatement("UPDATE form_encounter SET " .
           //     "last_stmt_date = ?, stmt_count = stmt_count + 1 " .
           //     "WHERE id = ?", array($today, $row['id']));
           //ALB This is for debugging below
             $tempwhere .= " OR (id = " . $row['id'] . " AND pid = " . $row['pid'] . " AND encounter = " . $row['encounter'] . ")";
             ++$tempcount;
...
//ALB And debugging here
    if (!empty($tempwhere)) {
        $tempwhere = substr($tempwhere, 4);
        $tempwhere = '( ' . $tempwhere . ' )';
    }

//ALB Copied all of the below
fwrite($fhprint,"UPDATE form_encounter SET " .
                "last_stmt_date = " . $today . ", (stmt_count = stmt_count + 1) " .
                "WHERE " . $tempwhere);
fwrite($fhprint, "<br />Total number of updates: " . $tempcount . ".");
if ($tempcount > 50) { //ALB Don't run the query if > 50 updates
    fwrite($fhprint, "<br /><br />Note: Over 50 updates. Will not perform update query.");
}

//ALB Only now updating statement numbers and dates, do not update if > 50 updates
if (!$DEBUG && !$_REQUEST['form_without']) {
   if ($tempcount <= 50) {
            sqlStatement("UPDATE form_encounter SET " .
                "last_stmt_date = CURRENT_DATE(), stmt_count = (stmt_count + 1) " .
                "WHERE " . $tempwhere);
   }
}
            ...
            <form id="formSearch" action="" enctype='multipart/form-data' method='post'>
                        ...
                        <div class="form-row p-2">
                            ...
                            <div class="form-group col-lg" style="padding-right:0px">
                                ...
                                <select name='form_category' id='form_category' class='form-control'>
                                    <?php
                                    //ALB Added new categories here
                                    foreach (array(xl('Open'), xl('All'), xl('Due Ins'), xl('Due Pt'), xl('Due Pt (New)'), xl('Due Pt (>30d)'), xl('Owe Pt'), xl('Owe Pt (New)')) as $value) {
                ...
                <fieldset id="search-results" class="oe-show-hide px-2">
                    ...
                    <div class="table-responsive">
                        ...
                        if (!empty($_REQUEST['form_search']) || !empty($_REQUEST['form_print'])) {
                            ...
                            if ($eracount) {
                                ...
                                //ALB Change here to display any combination of starting and ending date
                                if ($form_date) {
                                    if ($where) {
                                        $where .= " AND ";
                                    }
				    ...
				    if ($where) {
                                    ...
                                    $where .= "f.date <= '" . add_escape_custom($form_to_date) . "'";
                                }
                        ...
                        <table class="table table-striped table-sm">
                            <thead>
                            <tr>
                                <th class="id dehead"><?php echo xlt('PID'); ?></th> <!--ALB Changed from Billing note -->
                                <th class="dehead">&nbsp;<?php echo xlt('Patient'); ?></th>
                                <th class="dehead">&nbsp;<?php echo xlt('Invoice'); ?></th>
                                <th class="dehead">&nbsp;<?php echo xlt('Svc Date'); ?></th>

  				<!-- ALB Adding primary insurance here -->
 				<th class="dehead">&nbsp;<?php echo xlt('Primary Ins'); ?></th>
                            ...
                            while ($row = sqlFetchArray($t_res)) {
                                ...
                                //ALB Adding isowed category
                                $isowedpt = ($duncount >= 0 && $isowedany) ? " checked" : "";
                                $isdueorowedpt = ($isduept == " checked" || $isowedpt == " checked") ? " checked" : ""; 

                                //ALB Skip invoices not in the "Owed" category.
                                if (substr($_POST['form_category'], 0, 3) == 'Owe' && (!$isowedany || $duncount < 0)) continue;
                                if ($_POST['form_category'] == 'Owe Pt (New)' && (!$isowedany || $duncount > 0)) continue;
                                ...
                                //ALB Adding some additional categories
      				...
      				//ALB For New statements, skip the ones that have been billed previously, but calculate total of those that have been previously sent out.
      				if ($_POST['form_category'] == 'Due Pt (New)') { 
        				if ($last_stmt_date != '') continue;
      				}

      				//ALB Skip invoices that are <=30 days since last statement in the Due Pt(>30d) category (but calculate total balance). Also, if in collections, skip these over 30 days; otherwise, they'll keep accumulating in that report.
      				...
      				//ALB Getting primary insurance info
      				$temp_sql = SQLStatement("select ic.name from insurance_companies as ic " .
    					"left join insurance_data as insd on ic.id = insd.provider " .
    					"where insd.pid = " . $row['pid'] . " and insd.date <= '" . $svcdate . "' and " .
    					"insd.type='primary' order by insd.date DESC limit 1");
     				$temp_res = sqlFetchArray($temp_sql);
   				$pt_ins = $temp_res['name'];
     				if (!$pt_ins || $pt_ins == '') $pt_ins = "Self-Pay";
  				    ...
  				    <!--ALB Adding pt insurance here -->
  				    <td class="detail">&nbsp;<?php echo $pt_ins ?></td>
                                    ...
                                    <?php if (!$eracount) { ?>
                                        <td class="detail text-left">
                                            ...
                                            <input type='checkbox'
                                                   name='form_cb[<?php echo attr($row['id']) ?>]'<?php echo text($isdueorowedpt); ?> />
                                            <?php
                                            if ($in_collections) {
