...
//ALB Added ability to pick an insurance
...
if (!empty($_POST['form_refresh']) || !empty($_POST['form_csvexport'])) {
    ...
    //ALB Added insurance to query. Also, added max and min value for reimbursement
    ...
    "c.financial_reporting, id.provider, round(max((b.fee - ar_act.adjust)/b.units),2) as MaxReimb, round(min((b.fee - ar_act.adjust)/b.units),2) as MinReimb " .
    ...
    "LEFT OUTER JOIN insurance_data AS id ON b.pid = id.pid AND id.type = 'primary' AND id.date <= date(b.date) " .
    ...
    //ALB Added insurance
    if ($form_payer_id) {
        $query .= " AND id.provider = ?";
        array_push($sqlBindArray, $form_payer_id);
    }
    ...
    while ($erow = sqlFetchArray($res)) {
        ...
        //ALB Added 2 lines here
        $row['MaxReimb'] = $erow['MaxReimb'];
        $row['MinReimb'] = $erow['MinReimb'];
...
<th >
        <?php echo xlt('Avg Reimbursement/Unit');  //ALB Added reimbursement per unit ?>
</th>
...
<th >
    ...
    foreach ($rows as $key => $row) {
        ...
        //ALB Added reimbursement per unit, including max and min for the next 2 lines
        $print = "<tr bgcolor='". attr($bgcolor) . "'><td class='detail'>".text($row['Procedure codes'])."</td><td class='detail'>".text($row['Units'])."</td><td class='detail'>".text(oeFormatMoney($row['Amt Billed']))."</td><td class='detail'>".text(oeFormatMoney($row['Paid Amt']))."</td><td class='detail'>".text(oeFormatMoney(($row['Amt Billed'] - $row['Adjustment Amt'])/$row['Units'])).' (Max: '. oeFormatMoney($row['MaxReimb']).','.' Min: '. oeFormatMoney($row['MinReimb']).')'."</td><td class='detail'>".text(oeFormatMoney($row['Adjustment Amt']))."</td><td class='detail'>".text(oeFormatMoney($row['Balance Amt']))."</td>";
        $csv = csvEscape($row['Procedure codes']) . ',' . csvEscape($row['Units']) . ',' . csvEscape(oeFormatMoney($row['Amt Billed'])) . ',' . csvEscape(oeFormatMoney($row['Paid Amt'])) . ',' . csvEscape(oeFormatMoney(($row['Amt Billed'] - $row['Adjustment Amt'])/$row['Units'])) . ',' . csvEscape(oeFormatMoney($row['Adjustment Amt'])) . ',' . csvEscape(oeFormatMoney($row['Balance Amt'])) . "\n";
    ...
    if (!$_POST['form_csvexport']) {
        ...
        //ALB Total reimbursement per unit
        echo " <td class='detail'>" .
        text(oeFormatMoney(($grand_total_amt_billed - $grand_total_amt_adjustment)/$grand_total_units)) . "</td>\n";
