...
if (empty($_POST['start']) || empty($_POST['end'])) {
    ...
    $startdate = date('Y-m-d', time()); //ALB Today's date by default   date('Y-m-d', (time() - 30*24*60*60));
...
<h2><?php echo text($facility['name'] ?? '')?></h2>
    ...
    <?php
        $sqlBindArray = array();
        //ALB Added date(date) below to change to short date form; wasn't working otherwise
