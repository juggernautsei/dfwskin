...
$where  = "e.pc_eid in($ids) AND c.eligibility_id IS NOT NULL";  //ALB Only show patients with insurances that have a valid elg id (otherwise, no elg checking with clearinghouse is enabled)
...
if ($exclude_policy != "") {
    ...
    //ALB Changed extension of file from .txt to .270 per Change Healthcare
    if (isset($_POST['form_savefile']) && !empty($_POST['form_savefile']) && $res) {
        header('Content-Type: text/plain');
        header(sprintf(
            'Content-Disposition: attachment; filename="batch-elig-270-%s-%s.270"',
            strtolower(str_replace(' ', '', $X12info['name'])),
            date("Y-m-d-H-i-s")
        ));
...
<html>
    ...
    <head>
        ...
        <title><?php echo xlt('Eligibility Inquiry Batch Creation'); ?></title>
        ...
        <script type="text/javascript">
            ...
            function deletetherow(id){
                var suredelete = true; //ALB Don't need to confirm every time    confirm(stringDelete);
                if(suredelete == true){
    ...
    <body class="body_top">
        ...
        <span class='title'><?php echo xlt('Report'); ?> - <?php echo xlt('Eligibility Inquiry Batch (EDI-270) Creation'); ?></span>
        ...
        <form method='post' name='theform' id='theform' action='edi_270.php' onsubmit="return top.restoreSession()">
                                       ...
                                       <td>
                                        ...
                                        <td>
                                            <select name='form_x12' id='form_x12' class='form-control' onchange='return toggleMessage("emptyVald","form_x12");'>
                                                <!--ALB not needed <option value=''>--<?php echo xlt('select'); ?>--</option>-->
                                                <?php
                                        ...
                                        </td><!--ALB Changed this  <td> 
                                                <span id='emptyVald' style='color:red;font-size:12px;visibility: <?php echo $_POST['form_payer_id'] >0 ? "hidden" : ""; ?>'> *
                                                    <?php echo xlt('A single insurance company must be selected for EDI 270 batch creation.'); ?></span>
                                        </td> -->
                                    </tr>
                                </table>
                            </div>
                                    ...
                                    <td>
                                        <div class="text-center">
                                            <div class="btn-group" role="group">
                                                ...
                                                <a href='#' class='btn btn-default btn-transmit' onclick='validate_batch(false)'; >
            ...
            <div class='text'><!--ALB added second comment in the text below -->
                <?php echo xlt('Please choose date range criteria above, and click Refresh to view results. Note: Only patients whose insurance companies are set up for eligibility checking are displayed here.'); ?>
            </div>
