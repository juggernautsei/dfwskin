...
//ALB Adding/changing some values here
$is_due_ins     = ($_POST['form_category'] == 'Due Ins') || ($_POST['form_category'] == 'Due Ins (>30d)') || $is_ins_summary;
$is_open_ins    = ($_POST['form_category'] == 'Open Ins (>30d)');

//$is_due_ins     = ($_POST['form_category'] == 'Due Ins') || $is_ins_summary;
$is_due_pt      = substr($_POST['form_category'],0,6) == 'Due Pt'  ;
...
if (!empty($_POST['form_refresh']) || !empty($_POST['form_export']) || !empty($_POST['form_csvexport'])) {
   ...
   //ALB Don't need some of these to display by default
    $form_cb_ssn      = false;
    $form_cb_dob      = false;
    $form_cb_pubpid   = false;
    $form_cb_adate    = false;
        ...
        if ($ptrow['count'] > 1 && !$is_due_ins) {
          ...
          echo "  <td class='detotal' colspan='$final_colspan'>&nbsp;Total Patient Balance:</td>\n";
            ***************************************************************/
            ...
            if ($form_age_cols) {
                for ($c = 0; $c < $form_age_cols; ++$c) {
          	//ALB Getting rid of zeros in the Total Patient Balance cols 
          	$colamt = oeFormatMoney($ptrow['agedbal'][$c]);
         	if ($colamt == 0) $colamt = "";
          	echo "  <td class='detotal' align='right'>&nbsp;" . text($colamt) . "&nbsp;</td>\n";
            ...
            <td>
                <table>
                    ...
                    <tr>
                        ...
                        <?php
                        //ALB Added some more options here
                        foreach (array('Open' => xl('Open'),'Due Pt' => xl('Due Pt'),'Due Pt (>1 Stmt)' => xl('Due Pt (>1 Stmt)'),'Due Pt (4 Stmts)' => xl('Due Pt (4 Stmts)'),'Due Ins' => xl('Due Ins'),'Due Ins (>30d)' => xl('Due Ins (>30d)'),'Open Ins (>30d)' => xl('Open Ins (>30d)'),'Ins Summary' => xl('Ins Summary'),'Credits' => xl('Credits'),'All' => xl('All')) as $key => $value) {
...
if (!empty($_POST['form_refresh']) || !empty($_POST['form_export']) || !empty($_POST['form_csvexport'])) {
    ...
    $query = "SELECT f.id, f.date, f.pid, CONCAT(w.lname, ', ', w.fname) AS provider_id, f.encounter, f.last_level_billed, " .
      ...
      "p.postal_code, p.phone_home, p.phone_cell, p.ss, p.billing_note, " .
    ...
    while ($erow = sqlFetchArray($eres)) {
        ...
        //ALB Only want to see open insurance claims that have a charge associated with it
        if ($_POST['form_category'] == 'Open Ins (>30d)') {
          if ($erow['charges'] + $erow['sales'] == 0) continue;
        }
        ...
        //ALB Additional lines here
        if ($is_open_ins && $duncount >= 0) continue;
        if ($is_due_pt  && $pt_balance <  0) continue;

      //ALB - skip rows that are less than 2 statements if Due Pt (>1stmt) selected
      if ($_POST['form_category'] == 'Due Pt (>1 Stmt)') {
        if ($duncount < 2) continue;
        //ALB - also skip rows if in collections
        $in_collections = stristr($erow['billing_note'], 'IN COLLECTIONS') !== false;
        if ($in_collections) continue;
      }

      //ALB - skip rows that are less than 4 statements if Due Pt (4 stmts) selected
      if ($_POST['form_category'] == 'Due Pt (4 Stmts)') {
        if ($duncount < 4 || $pt_balance < 0) continue;
        //ALB - also skip rows if in collections
        $in_collections = stristr($erow['billing_note'], 'IN COLLECTIONS') !== false;
        if ($in_collections) continue;
      }

      //ALB - skip rows that are <30 days old if Due Ins (>30 d) or Open Ins (>30d) is selected
      if ($_POST['form_category'] == 'Due Ins (>30d)' || $_POST['form_category'] == 'Open Ins (>30d)') { 
        if ($svcdate == '') continue;
        $diff = strtotime(date('Y-m-d')) - strtotime($svcdate); 
        ...
        //ALB Use cell phone with home phone as backup
        $row['phone']     = ($erow['phone_cell'] <> "" ? $erow['phone_cell'] : $erow['phone_home']);
    ...
    if ($_POST['form_export']) {
        ...
        if (true) {
            //ALB - Simplified version to include only necessary columns for balance calls
            ...
            if ($form_cb_phone) {
                echo csvEscape(xl('Phone Number')) . ',';
            }
            ...
            echo csvEscape(xl('Encounter ID')) . ',';
  ...
  <td class="detail" align="center">
          ...
          // Added zero balances for a complete spreadsheet view
            ...
            if (
                ...
                // ALB - Will simplify the output to only include few columns for balance calls  
                //echo csvEscape($row['ins1'])                         . ','; // insname
  
                //ALB For phone calls, only need one name, don't have to repeat it
                echo csvEscape(($ptname == $prevpt) ? '' : $ptname)                     . ',';
                ...
                //ALB For phone calls, only need one phone number, don't have to repeat it
                if ($form_cb_phone) {
                    echo csvEscape(($ptname == $prevpt) ? '' : $row['phone'])                       . ',';
                ...
                //ALB For CSV report, don't need to repeat same patient, so they don't get several calls at once
                $prevpt = $ptname;
