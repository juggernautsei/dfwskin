...
class X125010837P
{
    public static function genX12837P($pid, $encounter, &$log, $encounter_claim = false)
    {
        ...
        "*" . $claim->groupNumber() .
        "*" . '' . //ALB Don't need groupname for now ($claim->groupNumber() ? '' : $claim->groupName()) . // if groupNumber no groupName
        ...
        //ALB Change here to bring in resubmission code   
        //$claim->frequencyTypeCode() .
        ($claim->medicaidResubmissionCode() ? $claim->medicaidResubmissionCode() : $claim->frequencyTypeCode()) .
        "*" . "Y" .
        "*" . "A" .
        ...
        //ALB This is incorrect. Need to use medicaidoriginalreference instead of icn_resubmission_number
        ...
        if ($claim->medicaidOriginalReference()) {
            ...
            $out .= "REF" .
            "*" . "F8" .
            "*" . $claim->medicaidOriginalReference() .
            "~\n";
        }

        if ($claim->cliaCode()) { //ALB Put CLIA number here for all payers   && ($claim->claimType() === 'MB')) {
        ...
        if ($claim->facilityPOS() != 12  && ($claim->facilityID() != $claim->billingFacilityID())) {    //ALB NPI can be the same for different facilities ($claim->facilityNPI() != $claim->billingFacilityNPI())) {
            ++$edicount;
        ...
        for ($ins = 1; $ins < $claim->payerCount(); ++$ins) {
            ...
            "*" . '' .  //ALB Don't need groupname here ($claim->groupNumber() ? '' : $claim->groupName()) .
