...
// ISA Segment  - EDI-270 format
    ...
    public static function createISA($row, $X12info, $segTer, $compEleSep)
    {
        ...
        $ISA[1] = "03";  // Author Info Qualifier ALB - Changed from 00 per ChangeHealthcare
        $ISA[2] = str_pad("0000000", 10, " ");        // Author Information
        $ISA[3] = "01"; //   Security Information Qualifier ALB - Changed from 00 per ChangeHealthcare
        //   MEDI-CAL NOTE: For Leased-Line & Dial-Up use '01',
        //   for BATCH use '00'.
        //   '00' No Security Information Present
        //   (No Meaningful Information in I04)
        //ALB Added info for edi 270
...
// EDI-270 Batch file Generation
    public static function printElig($res, $X12info, $segTer, $compEleSep)
    {
        ...
        $PATEDI = "";
        // For Header Segment
        $overallCounter = 1; //ALB Need this an a counter for the whole batch file
        ...
        $refiden = 5432101;
        foreach ($res as $row) {
            if ($overallCounter == 1) { //ALB Only 1 header for the batch file, but there could be different payers within the batch, so changed below to be able to add more HL segments
            //if ($nHlCounter == 1) {
                // create ISA
                ...
                $overallCounter = $overallCounter + 1; //ALB
    ...
    <table class='table table-striped table-hover'>
        ...
        foreach ($res as $row) {
            ...
            echo "	<tr id='PR" . $i . "_" . text($row['policy_number']) . "'>
				...
				<td class ='detail'>" . text($row['lname']." ". $row['fname']) ."</td> <!--ALB Changed the next 4 lines to show patient info, not subscriber -->
				<td class ='detail'>" . text($row['dob']) . "</td>
				<td class ='detail'>" . text($row['sex']) . "</td>
				<td class ='detail'>" . text($row['ss']) . "</td>
				<td class ='detail'>
...
// To Show Eligibility Verification data
    ...
    {
        $query =
            "SELECT DISTINCT eligr.*, eligv.insurance_id, eligv.copay, insd.pid, insc.name, " .  //ALB Added distinct here
        ...
        while ($benefit = sqlFetchArray($result)) {
            ...
            $color = "";
            switch ($benefit['type']) { //ALB Changed below
              ...
              case 'F': //Limitations
                $color = "yellow";
                break;
              case 'G': //ALB Added this one --OOP Max
                $color = "purple";
                break;
            }
        ...
        $showString .= (!empty($benefit['percent']) && $benefit['type'] == 'A') ? "<b>" . xlt('Percentage') . ":</b> " . text($benefit['percent']*100). "%" . "<br />\n" : '';
...
// Loop through each 271. '\n' delims records in batch.
        foreach ($responses as $new) {
            ...
            foreach ($segments as $segment) {
                ...
                switch ($elements[0]) {
                    ...
                    } elseif ($elements[1] == "03") { //ALB - Dependent (actual patient, if not the subscriber)
                        $in['lname'] = $elements[3];
                        $in['fname'] = $elements[4];
                        $in['mname'] = $elements[5];
                    }
                    ...
                    case 'DMG':
                        ...
                        if (!(int)$in['pid']) { //ALB Changed from subscriber_fname, etc to fname, in case the dependent, not subscriber, is the patient
                    ...
                    case 'REF':
                        if ($elements[1] == "EJ") {
                            ...
                            if (!$in['pid']) { //ALB Change from subscriber_fname to fname, in case this is a dependent
                                $in['pid'] = (int)self::getPatientMatch(
                                    $in['fname'],
                                    $in['lname'],
                                    $in['sex'],
                    ...
                    if ($elements[4] == 'MA') break; //ALB Skip Medicare part A
                        ...
                        $eb = array(
                            ...
                            'coverage_type' => $subelement3 ?  $codes->get_271_code("EB03", $subelement3) :$subelement3,
                            ...
                            'amount' => ($elements[7] || $elements[7] == '0') ? number_format($elements[7], 2, '.', '') : '', //ALB I changed to display 0 as well
                            'percent' => $elements[8],
                    ...
                    case 'MSG':
                        $bcnt = count($benefits) - 1;
                        if ($bcnt > -1) {
                            $benefits[$bcnt]['message'] = $benefits[$bcnt]['message'] ? $benefits[$bcnt]['message'] : $elements[1]; //ALB Changed here
                            //$benefits[$bcnt]['message'] .= $elements[1];
                        }
                        break;

                    case 'SE':
                        //ALB It looks like this is needed, but the one for segment TRN is not, as they duplicate each other.
                        $in['benefits'] = $benefits ? $benefits : [];

                        if (!$in['pid']) { //ALB Added this and changed from subscriber_fname to fname, in case this is a dependent
                         $in['pid'] = (int)self::getPatientMatch(
                             $in['fname'],
                             $in['lname'],
                             $in['sex'],
    ...
    public static function makeEligibilityReport($subscribers = [])
    {
        ...
        $binfo .= xlt("Subscriber") . ": " . $subscriber['subscriber_fname'] . " " . $subscriber['subscriber_lname'] . " " . $subscriber['subscriber_mname'] .
            nl2br ("\r\n") .xlt("Member Id") . ": " . $subscriber['subscriber_member_id'] .
            nl2br ("\r\n") .xlt("Patient Id") . ": " . $subscriber['pid'] . nl2br ("\r\n"); //ALB Changed this
            ...
            foreach ($subscriber['benefits'] as $key => $benefits) {
                foreach ($benefits as $key => $benefit) {
                    if ($key == 'type') {
                        $binfo .= nl2br ("\r\n"); //ALB This didn't work "\n";
                    }
                    if (empty($benefit) || $key == 'type') {
                        continue;
                    }
                    $binfo .= "\t" . ucwords(str_replace('_', ' ', $key)) . ": " . $benefit . nl2br ("\r\n"); // ALB This didn't work"\n";
                }
            }
        }
