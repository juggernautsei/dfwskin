...
class InvoiceSummary
{
    public static function arGetInvoiceSummary($patient_id, $encounter_id, $with_detail = false)
    {
        ...
        $res = sqlStatement("SELECT " .
            ...
            "a.post_time, a.session_id, a.sequence_no, a.account_code, a.follow_up_note, a.claim_number, " .
        ...
        while ($row = sqlFetchArray($res)) {
            ...
            if ($with_detail) {
                ...
                $tmp = array();
                $paydate = substr($row['post_time'], 0, 10); //ALB Just post time is fine here   empty($row['deposit_date']) ? substr($row['post_time'], 0, 10) : $row['deposit_date'];
                if ($row['pay_amount'] != 0) {
                ...
                //ALB Not sure why, but it was commented out $tmp['insurance_company'] = substr(($row['name'] ?? ''), 0, 10);
                if ($ins_id) {
                    $tmp['ins'] = $ins_id;
                }
            ...
            //ALB Keeping a running total for patient pmts, adjustments, and pmt methods
            if ($tmp['plv'] == 0) {
            ...
            //ALB Added claim number here
            $tmp['claim_number'] = $row['claim_number'];
