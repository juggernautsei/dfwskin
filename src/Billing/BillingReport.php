...
class BillingReport
{
        ...
        if (isset($_REQUEST['final_this_page_criteria'])) {
            foreach ($_REQUEST['final_this_page_criteria'] as $criteria_key => $criteria_value) {
                ...
                if (strpos($criteria_value, "billing.billed|=|1") !== false) {
                 ...
                 //ALB This paragraph is all new
                 }  elseif(strpos($criteria_value,"billing.payer_id")!== false) {
                 //ALB If encounter hasn't been billed, need to look up the insurance company on the date of the encounter
                 //ALB change query if doing an exclude (insurance is not...
                 if (strpos($criteria_value,"=") <> 0) {
                  //This is an include query
                  $temp_value = substr($criteria_value,strpos($criteria_value,"=")+2);
                  $query_part .= ' AND (' . $criteria_value . ' OR (IsNull(billing.payer_id) AND insurance_data.provider = ' . $temp_value . ' AND insurance_data.date <= form_encounter.date AND NOT EXISTS (SELECT * FROM insurance_data AS ins2 WHERE insurance_data.pid = ins2.pid AND insurance_data.type = ins2.type AND ins2.date > insurance_data.date AND ins2.date <= form_encounter.date)))';
                 } else {
                  //This is an exclude query
                  $temp_value = substr($criteria_value,strpos($criteria_value,"<")+3);
                  $query_part .= ' AND ((!IsNull(billing.payer_id) AND ' . $criteria_value . ') OR (IsNull(billing.payer_id) AND insurance_data.provider <> ' . $temp_value . ' AND insurance_data.date <= form_encounter.date AND NOT EXISTS (SELECT * FROM insurance_data AS ins2 WHERE insurance_data.pid = ins2.pid AND insurance_data.type = ins2.type AND ins2.date > insurance_data.date AND ins2.date <= form_encounter.date)))';
                 } 
    ...
    public static function getBillsBetweenReport(
        ...
        //ALB - Changed to DESC order by encounter date.
        $sql = "SELECT distinct form_encounter.date AS enc_date, form_encounter.pid AS enc_pid, " .
            "form_encounter.encounter AS enc_encounter, form_encounter.provider_id AS enc_provider_id, billing.* " .
            "FROM form_encounter " .
            "LEFT OUTER JOIN billing ON " .
            ...
            "LEFT OUTER JOIN insurance_data on insurance_data.pid = form_encounter.pid and insurance_data.type = 'primary' " .
            "WHERE 1=1 $query_part  " . " $auth " . " $billstring " .
            "ORDER BY enc_date DESC, form_encounter.provider_id, form_encounter.encounter, form_encounter.pid, billing.code_type, billing.code ASC";
        //echo $sql;
        $res = sqlStatement($sql, array($code_type));
        $all = false;
        for ($iter = 0; $row = sqlFetchArray($res); $iter++) {
    ...
    //ALB New query
    $sql = "select distinct $cols " .
    "from form_encounter, billing, patient_data, claims, insurance_data, insurance_companies where " .
    "billing.encounter = form_encounter.encounter and " .
    "billing.pid = form_encounter.pid and " .
    "patient_data.pid = form_encounter.pid and " .
    "claims.patient_id = form_encounter.pid and claims.encounter_id = form_encounter.encounter and ".
    "insurance_data.pid = form_encounter.pid and insurance_data.type = 'primary' AND ".
