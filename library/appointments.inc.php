...
//ALB Added a parameter here
function fetchEvents($from_date, $to_date, $where_param = null, $orderby_param = null, $tracker_board = false, $nextX = 0, $bind_param = null, $query_param = null, $skip_cancellations = false)
{
    ...
    if ($query_param) {
        ...
        if ($nextX) {
          ...
          //////
            ...
            //ALB Skip canceled or rescheduled appointments
            if ($skip_cancellations) {
                $where .= " AND e.pc_apptstatus <> 'x' AND e.pc_apptstatus <> '<>'";
            }
        ...
        //ALB Added several fields to the appointment report, such as DOB, SSN, Insurance name, and policy #, facility id and nickname
        $query = "SELECT " .
        "e.pc_eventDate, e.pc_endDate, e.pc_startTime, e.pc_endTime, e.pc_duration, e.pc_recurrtype, e.pc_recurrspec, e.pc_recurrfreq, e.pc_catid, e.pc_eid, e.pc_gid, e.pc_apptstatus," .
        "e.pc_title, e.pc_hometext, e.pc_apptstatus, " .
        "p.fname, p.mname, p.lname, p.pid, p.pubpid, p.phone_home, p.phone_cell, p.DOB," .
        "p.hipaa_allowsms, p.phone_home, p.phone_cell, p.hipaa_voice, p.hipaa_allowemail, p.email, " .
        "ic.name as insname, i.policy_number as policy, " .
        "u.fname AS ufname, u.mname AS umname, u.lname AS ulname, u.id AS uprovider_id, " .
        "f.name, f.id as facility_id, f.nickname, " .
        "$tracker_fields" .
        ...
        "LEFT OUTER JOIN insurance_data AS i ON (i.id = (SELECT id FROM insurance_data AS i WHERE i.pid = e.pc_pid AND type = 'primary' ORDER BY date DESC LIMIT 1)) " .
...
//ALB Added a parameter here
function fetchAllEvents($from_date, $to_date, $provider_id = null, $facility_id = null, $hide_canceled = false)
{
    $sqlBindArray = array();
    ...
    //ALB Do not include canceled appointments
    if ($hide_canceled) {
        $where .= " AND e.pc_apptstatus NOT IN ('x','<>','%')";
    }
...
function fetchAppointments($from_date, $to_date, $patient_id = null, $provider_id = null, $facility_id = null, $pc_appstatus = null, $with_out_provider = null, $with_out_facility = null, $pc_catid = null, $tracker_board = false, $nextX = 0, $group_id = null, $patient_name = null)
{
    ...
    if ($pc_appstatus != '') {
        if ($pc_appstatus == 'NO_SHOW') {
            //ALB Added changes here
            $where .= " AND e.pc_apptstatus NOT IN ('x','<>','%')";
        } else {
...
{
    //ALB Hide all canceled events here - last "true" below
    $appointments = fetchAllEvents($from_date, $to_date, $provider_id, $facility_id, true);
