...
function report_header_2($stmt, $providerID = '1')
{
    ...
    //ALB Only display billing facility location
    if ($facility['billing_location'] <> 1) {
    	...
    	//$billing_query = sqlStatement("SELECT * FROM facility where billing_location = 1 LIMIT 1);
    	$facility = sqlFetchArray($service_query);
    }
    ...
    <table style="width:100%;">
        <tr>
            ...
            <td align='left' style='<?php echo ($haveLogo ? text("width:40%;max-width:50%;") : text("width:50%;") ) ?>'> <!--adds some growing room-->
                <!--ALB <em style="font-weight:bold;font-size:1.4em;"><?php echo text($facility['name']); ?></em><br />  -->
                <?php echo str_repeat('&nbsp;', 10) . text($facility['name']); ?><br />
                <?php echo str_repeat('&nbsp;', 10) . text($facility['street']); ?><br />
                <?php echo str_repeat('&nbsp;', 10) . text($facility['city']); ?>, <?php echo text($facility['state']); ?> <?php echo text($facility['postal_code']); ?><br />
                <?php echo str_repeat('&nbsp;', 10) . xlt('Phone').': ' .text($facility['phone']); ?>
                <!-- ALB Don't need <?php echo xlt('Fax').': ' .text($facility['fax']); ?><br />  -->

            </td>
            <td align='left'>
                <?php 

 		//ALB Get statement number, so we can change the wording on it.
 		$stmtno = 0;
 		foreach ($stmt['lines'] as $line) {
		   $stmtnonext = $line['notice'];
		   if ($stmtnonext > $stmtno) $stmtno = $stmtnonext;
		}
		...
		if ($stmtno < 2 || $stmt['amount']<0) {
		 ...
		 echo text($stmtnotice1); 

                ?><br />
                <?php echo text($stmtnotice2); ?><br />
                <?php echo text($stmtnotice3); ?><br />
                <?php echo text($stmtnotice4); ?>

                <!--ALB <em style="font-weight:bold;font-size:1.4em;"><?php echo xlt('Patient: ') . text($titleres['fname']) . " " . text($titleres['lname']); ?></em><br />
                <b style="font-weight:bold;"><?php echo xlt('Account Number'); ?>:</b> <?php echo text($stmt['pid']); ?><br />
                <b style="font-weight:bold;"><?php echo xlt('Statement Date'); ?>:</b> <?php echo text(oeFormatShortDate()); ?><br />
...
// Billing location modified by Daniel Pflieger at Growlingflea Software
    ...
    // Text only labels
    //ALB Changed some labels
    $label_addressee = xl('ADDRESSEE');
    $label_remitto = xl('REMIT TO');
    $label_chartnum = xl('Account Number');
    $label_insinfo = xl('<u>') . xl('Insurance information on file:') . xl('</u>');
    $label_totaldue = xl('Balance Due');
    $label_payby = xl('If paying by');
    $label_cards = xl('credit card');
    $label_card_action = xl(', please fill out below or pay online 24/7 at <u>dfwskinsurgery.com/billpay</u>'); //ALB Added this
    $label_cardnum = xl('Card Number');
    $label_expiry = xl('Exp Date');
    $label_cvv = xl('CVV/Security Code');
    ...
    $label_amt = xl('Amount');
    $label_call = xl('Keep the above portion for your records.');
    $label_dept = xl('Billing Department');

    //ALB Addl labels
    $label_nextpage = xl('(continued on next page)');
    $label_prevpage = xl('(continued from previous page)');
    $label_paydate = xl('Balance due upon receipt');
    $label_ptname = xl('Patient');
    $label_statdate = xl('Statement Date');
    $label_qty = xl('Qty');
    $label_prev_due = xl('Additional Outstanding Balance');
    $label_due = xl('Current Statement Balance');
    $label_total_due = xl('Total Balance');
    $label_checks_payable = xl('Please make checks payable to: ') . $clinic_name . ".";

    $prevbalance = get_patient_balance($stmt['pid'],false) - $stmt['amount'];
    $totalbalance = $prevbalance + $stmt['amount'];
    $stmtamount = $totalbalance;

 //ALB If negative balance, we owe the patient, so change labels
 if ($totalbalance < 0) {
    $stmtamount = -$totalbalance;
    ...
    $label_payby = xl('');
    $label_cards = xl('');
    $label_card_action = xl('');  
    $label_cardnum = xl('');
    $label_expiry = xl('');
    $label_seccode = xl('');
    $label_sign = xl('');
    $label_retpay = xl('     Refund check enclosed. We appreciate your business.');
    ...
    $out  = "<div style='margin-left:40px;margin-top:20px;'><pre>";
...
$out .= sprintf("_________________________________________________________________________________\n");
$out .= "\n";
$out .= sprintf("%-40s | %s: %-s\n",$label_addressee,$label_ptname,substr($stmt['patient'],0,20));
$out .= sprintf("%-40s | %s: %-s\n",null,$label_statdate, oeFormatShortDate($stmt['today']));
$out .= sprintf("%-40s | %s: %-s\n",$legal_addressee,$label_chartnum,$stmt['pid']);
$out .= sprintf("%-40s | %s: %-s\n",$stmt['to'][1],$label_totaldue,'$'.oeFormatMoney($stmtamount));
$out .= sprintf("%-40s | %s\n",$stmt['to'][2],$label_paydate);

$out .= sprintf("_________________________________________________________________________________\n");
$out .= "\n";
$out .= sprintf("_______________________________ %s _______________________________\n",$label_pgbrk);
$out .= "\n";
$out .= sprintf("%-12s %-55s %-3s %8s\n",$label_visit,$label_desc,$label_qty,$label_amt);
$out .= "\n";
...
if ($stmt['message']) {
  ...
  $count = 26;
    ...
    foreach ($stmt['lines'] as $line) {
        ...
        if ($GLOBALS['use_custom_statement']) {
            //ALB Trimming line description if too long.
            if(strlen($line['desc'])>53) {
               $description = substr($line['desc'],0,50)."...";
            } else { 
        ...
        $tmp = substr($description, 0, 3);
        if ($tmp == '992') $description = substr($description, 0, 5) . ": Office Visit, Level " . substr($description, 4, 1);
        ...
        foreach ($line['detail'] as $dkey => $ddata) {
            ...
            $proccode = substr($line['desc'], 0, 5);  //ALB Getting procedure code
            ...
            if ($ddata['pmt']) {
                ...
                $desc = $payer . xl('/Pmt') .' '. oeFormatShortDate($ddate) .': '. $ddata['src'].' '. $ddata['pmt_method'].' '. $ddata['insurance_company'];
            } else if ($ddata['rsn']) {
                if ($ddata['chg']) {
                    $amount = sprintf("%.2f", $ddata['chg']);
                    $desc = $payer . xl('/Adj') .' '.  oeFormatShortDate($ddate) .': ' . $ddata['rsn'].' '.$ddata['pmt_method'].' '. $ddata['insurance_company'];
                } else {
                    //ALB Actually, will output notes, but here's where to change it    continue; //ALB Will not output Notes for now
                    continue;
                    //$desc = $payer . xl('/Note') .' '. oeFormatShortDate($ddate) .': '. $ddata['insurance_company']. $ddata['rsn'].' '.$ddata['pmt_method'].' '. $ddata['insurance_company'];
                }
            } else if ($ddata['chg'] < 0) {
                $amount = sprintf("%.2f", $ddata['chg']);
                $desc = xl('Pt Paid');
                ...
                $amount = sprintf("%.2f", $ddata['chg']);
                $desc = $description;
            //ALB Getting the number of units
	    ...
	    //ALB Trimming line description if too long.
               if(strlen($desc)>53) $desc = substr($desc,0,50)."...";

            $out .= sprintf("%-12s %-55s %-3s %8s\n", oeFormatShortDate($dos), $desc, $units, $amount);
            ++$count;

            if ($prev_dos <> $dos) {

              $Ins1 = getInsuranceNameByDate($stmt['pid'], $dos, 'primary');
              $Ins2 = getInsuranceNameByDate($stmt['pid'], $dos, 'secondary');
              $Ins3 = getInsuranceNameByDate($stmt['pid'], $dos, 'tertiary');

              if ($Ins1 <> '' && $Ins1 <> $Prev_Ins1) {
                $out .= sprintf("%-12s %-55s %-3s %8s\n", '', $label_insinfo, '', '');
                ++$count;
                $ins1_desc = "<u>Primary Insurance:</u> " . substr($Ins1, 0, 34);
                $out .= sprintf("%-12s %-55s %-3s %8s\n", '', $ins1_desc, '', '');
                ++$count;
                $Prev_Ins1 = $Ins1;
              }
              if ($Ins2 <> '' && $Ins2 <> $Prev_Ins2) {
                $ins2_desc = "<u>Secondary Insurance:</u> " . substr($Ins2, 0, 34);
                $out .= sprintf("%-12s %-55s %-3s %8s\n", '', $ins2_desc, '', '');
                ++$count;
                $Prev_Ins2 = $Ins2;
              }
              if ($Ins3 <> '' && $Ins3 <> $Prev_Ins3) {
                $ins3_desc = "<u>Tertiary Insurance:</u> " . substr($Ins3, 0, 34);
                $out .= sprintf("%-12s %-55s %-3s %8s\n", '', $ins3_desc, '', '');
                ++$count;
                $Prev_Ins3 = $Ins3;
              }
            ...
            //ALB Page break if bill is too long, reset line count
            if ($count >= 61) {
                $out .= "\n";
                $out .= sprintf("%-12s %-40s\n", null, $label_nextpage);
                $out .= "\f";
 ...
 //ALB If the bill doesn't actually span 2 pages, but is still long enough to cut off part of the bottom portion, still need a page break here.
 if ($count > 41) {
     while ($count++ < 61) $out .= "\n"; //sprintf("%-10s %-40s  %-3s %8s\n", null, null, null, null);
     $out .= "\n";
     $out .= sprintf("%-11s %-40s\n", null, $label_nextpage);
     $out .= "\f";
     $out .= "\n";
    ...
    //ALB Change here, see next paragraph
    //$out .= "\n";
    //$out .= sprintf(
    //   "%-s: %-19s %-s: %-13s %-s: %8s\n",
    //    $label_ptname,
    ...
    //ALB Displaying current and previous outstanding balance, if any not shown on this statement
    $out .= sprintf("%71s: %8s\n", $label_due, oeFormatMoney($stmt['amount']));
    $out .= sprintf("%71s: %8s\n", $label_prev_due, oeFormatMoney($prevbalance));
    $out .= sprintf("%71s: %8s\n", $label_total_due, oeFormatMoney($prevbalance + $stmt['amount']));
...
//    }
    ...
    $out .= '</div>';
    $out .= '<div style="width:7.0in;border-top:1pt dotted black;margin:0px;"></div>';
    //$out .= '<div style="width:7.0in;border-top:1pt dotted black;font-size:12px;margin:0px;"></div>';
    $out .= sprintf("%-10s %s\n", '', $label_retpay);

    $out .= " <table><tr>"; //  style='width:7.0in;margin:auto;'><tr>";
    //$out .= '<td style="margin:auto;"></td>
    $out .= '<td style="width:3.0in;"><b>'
         . $label_ptname . ": " . substr($stmt['patient'],0,20) . '</b><br />'
         . $label_statdate . ": " . oeFormatShortDate($stmt['today']) . '<br />'
         . $label_chartnum . ": " . $stmt['pid'] . '<br />'
         . $label_totaldue . ': $' . oeFormatMoney($stmtamount);
    ...
    //    .$stmt['to'][2].'
     ...
     $out .= '</td><td style="width:0.5in;"></td>
  ...
  //ALB Skip if refund
   if ($totalbalance < 0) {
    $out .= '<div><table><tr><td>'; // style="width:7in;margin-left:20px;"><tr><td>'; // style="width:4.5in;">';
    $out .= "<br />";
    $out .= "<br />";
    $out .= "<br />";
    $out .= "<br /><br />";
    ...
    $out .= '<div><table><tr><td>'; // style="width:7in;margin-left:20px;"><tr><td>'; // style="width:4.5in;">';
    $out .= $label_checks_payable;
    $out .= "<br />";
    $out .= $label_payby.' '.$label_cards . $label_card_action;
    $out .= "<br /><br />";
    $out .= $label_cardnum .': _________________________________  '.$label_expiry.': ____ / ____ '.$label_cvv.': ______<br /><br />';
    $out .= $label_sign .'  ______________________________________________<br /><br />';
...
function create_statement($stmt)
{
    ...
    $label_cardnum = xl('Card Number');
    $label_expiry = xl('Exp Date');
    $label_cvv = xl('Security Code/CVV');
    $label_sign = xl('Signature');
...
// Billing location
    ...
    foreach ($stmt['lines'] as $line) {
        if ($GLOBALS['use_custom_statement']) {
            $description = substr($line['desc'], 0, 40);
        } else {
            $description = $line['desc'];
        }
        ...
        foreach ($line['detail'] as $dkey => $ddata) {
            ...
            //ALB Trimming line description if too long.
            if(strlen(txt($desc))>40) $desc = substr(txt($desc),0,37) . "...";
