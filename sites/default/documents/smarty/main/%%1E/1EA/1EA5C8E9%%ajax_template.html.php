...
foreach ($facilities as $f){
  //ALB Display facility nickname instead of name - function added to library/patient.inc
  echo "   <tr>\n";
  echo "   <td>\n";
  echo "   <div class='view1 font-weight-bold text-ovr-dark p-1 rounded' style='background-color:".$f['color'].";'>" . text(getFacilityNickname($f['id'])) . "</div>";
  echo "   </td>\n";
  echo "   </tr>\n";
}
 ?>
    ...
    foreach ($A_EVENTS as $date => $events) {
            ...
            foreach ($events as $e1) {
                   ...
                   || (($e1Start < $slotstartmins) && ($e1End > $slotendmins))) // span
                {
					...
					if($e1['catid'] == 3)
					{
						...
						//ALB This seems to overlap last appt and OUT  array_unshift($events_in_timeslot, $e1['eid']);
					}
                }
        ...
        foreach ($events as $event) {
            ...
            if($groupid)
                ...
                //ALB Added Provider: explanation here
                $divTitle = xl('Provider: ') . $provider["fname"] . " " . $provider["lname"];
                $result = sqlStatement("SELECT nickname,name,id,color FROM facility WHERE id=(SELECT pc_facility FROM openemr_postcalendar_events WHERE pc_eid=?)", [$eventid]);
                $row = sqlFetchArray($result);
                $color=$event["catcolor"];
                if($GLOBALS['event_color']==2) $color=$row['color'];
                //ALB Changed to nickname from name here and added description
                $divTitle .= "\n" . xl('Facility: '). $row['nickname'];

            if ($catid == 2 || $catid == 3 || $catid == 4 || $catid == 8 || $catid == 11 || $catid == 1) {
                ...
                //ALB Added the same for cat1 (meeting)
                else if ($catid == 1) $catname = xl("MEETING");

                //ALB Added explanation here
                $atitle = 'Category: ' . $catname;
                ...
                if($groupid)
                    ...
                    //ALB Changed order and added explanations here
                    $divTitle .= "\r\n" . xl('Category: ') . $catname . "\n" . xl('Patient Name: ') . $fname . " " . $lname . "\n" . xl('Comment: ') . $comment;
                ...
                if ($patientid) {
                    ...
                    //ALB Added a line break between patient DOB and comment below
       ...
       if($_SESSION['pc_facility'] == 0){
          ...
          if ($event['catid'] == 2) {
              //ALB Took out 20+ to make sure it works correctly
              $inTop = ($eStartPos - $timeslotHeightVal).$timeslotHeightUnit;  // +20
          ...
          // For "OUT" events, applying the background color in CSS.
            ...
            if ($event['catid'] != "6") {
              ...
              $background_string.
              //ALB Added a left border with the color of facility for the event
